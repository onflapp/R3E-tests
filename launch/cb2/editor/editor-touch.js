...
function initEditorTouch() {
  ...
  window.ontouchend = function(evt) {
    ...
    else if (el.classList.contains('act-remove-page')) {
      var file = el.dataset['file'] || el.closest('[data-file]').dataset['file'];
      window.sendWindowMessage({action:'UI_remove_page', file:file});
      evt.preventDefault();
      return false;
    }
    else if (el.dataset['file'] || el.closest('[data-file]')) {
      var file = el.dataset['file'] || el.closest('[data-file]').dataset['file'];
      loadFile(file);
      evt.preventDefault();
      return false;
    }
    else if (target.dataset['line'] || target.closest('[data-line]')) {
      var line = target.dataset['line'] || target.closest('[data-line]').dataset['line'];
      line = parseInt(line);
      if (line >= 0) {
    ...
    else if (editor.state.focused && evt.target.classList.contains('cm-task-open')) {
